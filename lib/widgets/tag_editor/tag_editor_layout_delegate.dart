...
class TagEditorLayoutDelegate extends MultiChildLayoutDelegate {
  ...
  void performLayout(Size size) {
    ...
    if (hasChild(textFieldId)) {
      ...
      positionChild(textFieldId, Offset.zero);
    }

    var cursor = Offset(0, textFieldSize.height + spacing);
    var tagSizes = <Size>[];
    ...
    for (final index in Iterable<int>.generate(length).toList()) {
      ...
      if (hasChild(getTagId(index))) {
        ...
        if (_isOverflow(
          ...
          cursor = Offset(0, cursor.dy + childSize.height + spacing);
    ...
    parentSize = Size(size.width, (tagSizes.isNotEmpty ? cursor.dy : 0) + textFieldSize.height);
  }
  ...
  void performLayout(Size size) {
    ...
    if (hasChild(textFieldId)) {
      ...
      // Forcing overflow behavior without checking
      textFieldSize = layoutChild(
        textFieldId,
        BoxConstraints.loose(Size.fromWidth(size.width)),
      );
      // Push the cursor down and back to the left
      cursor = Offset(0, cursor.dy + (tagSizes.isEmpty ? 0 : textFieldSize.height));
